...
for i = 1:numel(indexCellArray)
    ...
    g =abs(B -A*x0);%求残差
    [sorted_sums, top_three_indices] = maxk(sum(g, 1), 1);%找列范数最大的三列
...
%     [sortedVector,index1] = sort(g, 'descend');%每一列进行排序，降序
%    index = index1(1:100);  %选取每一列的最大值进行更新
    index = unique([ik'; top_three_indices_1]);%并集
